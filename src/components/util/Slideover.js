...
const Slideover = (props) => {
    ...
    const {opacity: overlayO, zIndex: overlayZ} = useSpring({
        from: {opacity: 0},
...
<div className={`fixed inset-0 overflow-hidden ${!isVisible && "pointer-events-none" }`}>
  <div className="absolute inset-0 overflow-hidden">
      {/* Overlay */}
    <animated.div onClick={handleClick} className="absolute inset-0 bg-gray-500" style={{opacity: overlayO, zIndex: overlayZ}} aria-hidden="true"></animated.div>
    <section className="absolute inset-y-0 right-0 pl-10 max-w-full flex" aria-labelledby="slide-over-heading">
        {/* Slide over panel */}
      <animated.div className="relative w-screen max-w-md pointer-events-auto" style={{transform: slideT.to([0,1], [0,width]).to(t => `translateX(${t}px)`)}}>
        <div className="absolute top-0 left-0 -ml-8 pt-4 pr-2 flex sm:-ml-10 sm:pr-4">
          <button onClick={handleClick} className="rounded-md text-gray-300 hover:text-white focus:outline-none">
        ...
        <div {...swipeHandler} className="h-full flex flex-col py-6 bg-white shadow-xl overflow-y-scroll">
            {resizeListener}
